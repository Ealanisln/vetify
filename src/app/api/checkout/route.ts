...
import { isStripeInLiveMode, getStripePriceIdForPlan } from '../../../lib/pricing-config';

export async function POST(request: NextRequest) {
  try {
    console.log('=== CHECKOUT DEBUG START ===');
    const { priceId, planKey, billingInterval = 'monthly' } = await request.json();
    console.log('1. Request body parsed:', { priceId, planKey, billingInterval });
    ...
    if (!actualPriceId && planKey) {
      ...
      console.log('1a. Resolved price ID from planKey:', planKey, '->', actualPriceId);
    }

    // Si priceId es un lookup key, convertirlo a price ID real
    if (actualPriceId && !actualPriceId.startsWith('price_')) {
      console.log('2. Converting lookup key to price ID:', actualPriceId);
    ...
    if (!actualPriceId) {
      console.error('Checkout error: Price ID or Plan Key is required');
